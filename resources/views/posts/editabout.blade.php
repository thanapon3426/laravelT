      ...
      <div class="col-sm-9 col-md-7 col-lg-5 mx-auto">
        <div class="card card-signin my-5">
          <div class="card-body">
            ...
            <form action="/updateabout/{{$post->id}}" method="post">
            {{ csrf_field() }}
            <input type="hidden" name="_method" value="PUT">
            <h4>TITLE</h4>
            <input class="btn btn-warning" type="text" name="title" value="{{$post->title}}">
            <h4>DESCRIPTION</h4>
            <textarea class="btn btn-warning" name="content" cols="30" rows="5" value="{{$post->content}}"> </textarea>
            <input type="submit" value="Save" class="btn btn-warning">
            </form>


            <form action="/destroyabout/{{$post->id}}" method="post">
            {{ csrf_field() }}
